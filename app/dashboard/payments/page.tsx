...
import { Calendar, DollarSign, Clock, CheckCircle, AlertCircle, Download, Upload } from "lucide-react"
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage payment batches and processing workflows</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">₹10,45,625</div>
              ...
              <div className="space-y-4">
                {selectedBatches.length > 0 && (
                  <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg">
                    <span className="text-sm font-medium">{selectedBatches.length} batch(es) selected</span>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center gap-4 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1">
                              <DollarSign className="h-3 w-3" />₹{batch.totalAmount.toLocaleString("en-IN")}
                            </span>
                            <span>{batch.invoiceCount} invoices</span>
                            <span className="flex items-center gap-1">
                          ...
                          <div className="text-xs text-muted-foreground">
                            Created by {batch.createdBy} • Approver: {batch.approver}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {batch.status === "Ready for Approval" && (
                          <Button size="sm" className="bg-secondary hover:bg-secondary/90">
                            Approve
                          </Button>
                        )}
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Upcoming payment due dates and schedules</CardDescription>
                </div>
                <div className="flex items-center gap-4">
                  <Select defaultValue="all">
                    <SelectTrigger className="w-40">
                ...
                {paymentSchedule.map((payment) => (
                  <div key={payment.id} className="p-4 border rounded-lg">
                    <div className="flex items-start justify-between">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-foreground">{payment.vendor}</span>
                          <Badge className={getStatusColor(payment.status)}>{payment.status}</Badge>
                        </div>
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          <span className="flex items-center gap-1">
                            <DollarSign className="h-3 w-3" />₹{payment.amount.toLocaleString("en-IN")}
                          </span>
                          <span className="flex items-center gap-1">
                            <Calendar className="h-3 w-3" />
                        ...
                        <div className="text-xs text-muted-foreground">
                          Invoices: {payment.invoiceNumbers.join(", ")}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Historical payment records and transactions</CardDescription>
                </div>
                <div className="flex items-center gap-4">
                  <Select defaultValue="all">
                    <SelectTrigger className="w-40">

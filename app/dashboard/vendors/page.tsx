    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage vendor relationships and information</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">₹17.5 Cr</div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Browse and manage your vendor database</CardDescription>
                </div>
                <div className="flex items-center gap-4">
                ...
                {vendors.map((vendor) => (
                  <div
                    ...
                    className={`p-4 border rounded-lg cursor-pointer transition-colors hover:bg-muted/50 ${
                      selectedVendor === vendor.id ? "border-primary bg-muted/30" : "border-border"
                    }`}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-4">
                        <Avatar className="h-12 w-12">
                          <AvatarImage src={`/abstract-geometric-shapes.png?height=48&width=48&query=${vendor.name}`} />
                          <AvatarFallback>
                            {vendor.name
                              .split(" ")
                              .map((n) => n[0])
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm text-muted-foreground">
                            ...
                            <div className="flex items-center gap-1">
                              <DollarSign className="h-3 w-3" />₹{(vendor.totalSpent / 100000).toFixed(1)}L total
                            </div>
                          </div>
                          <div className="flex items-center gap-4 text-xs text-muted-foreground">
                            <span>{vendor.invoiceCount} invoices</span>
                            <span>Last invoice: {new Date(vendor.lastInvoice).toLocaleDateString()}</span>
        ...
        <TabsContent value="performance" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Track vendor performance metrics and KPIs</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <TableBody>
                    {performanceData.map((vendor) => (
                      <TableRow key={vendor.vendor}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            {getRatingStars(vendor.qualityScore)}
                            <span className="text-sm ml-1">{vendor.qualityScore}</span>
                          </div>
                        </TableCell>
                        <TableCell>{vendor.responseTime}</TableCell>
                        <TableCell className="font-semibold">{vendor.invoicesProcessed}</TableCell>
                        <TableCell>{vendor.avgProcessingTime}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
        ...
        <TabsContent value="onboarding" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Track new vendor onboarding progress</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {onboardingData.map((item, index) => (
                  <div key={index} className="p-4 border rounded-lg">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-medium text-foreground">{item.vendor}</h3>
                        <p className="text-sm text-muted-foreground">Current Stage: {item.stage}</p>
                      </div>
